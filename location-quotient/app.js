...
(async () => {
"use strict";
const {el, txt, ticks, frame, hoverable, tableView, face, lead, SEQ, GRAY, INK} = PV;
...
/* HOW WIDE A STRING ACTUALLY PRINTS, in the viewBox's own units. PV.face() answers how
   TALL a line of a class is, which is what a stacked pair needs; a matrix also has to know
   how wide its widest cell reading is before it can choose a column pitch, and character
   count does not know the face — "22.7×" and "11.1×" differ by six units in the same
   class. Probed on the baseline for the same reason face() is: getComputedTextLength
   reports in viewBox coordinates. Keyed by the breakpoint as well as the class, because
   the shared sheet swaps the face at 760px and a cached width from the other side of that
   line is the same guess this whole file is trying to stop making. */
const strCache = new Map();
const strW = (svg, s, cls) => {
  const key = `${MOBILE.matches ? "s" : "w"}\u0000${cls}\u0000${s}`;
  if (strCache.has(key)) return strCache.get(key);
  const t = txt(svg, s, {x: 0, y: 0, class: cls, opacity: 0});
  const n = t.getComputedTextLength();
  svg.removeChild(t);
  strCache.set(key, n);
  return n;
};
...
/* THE RUNNER-UP IS THE RUNNER-UP OF A REAL RANKING. It used to be the second row of all
   six, which put rubber products there - a SLICE of plastics and rubber products, sitting
   in a list beside its own parent. Three of the six overlap or sit outside the cluster,
   so the only set that can be ranked is the three core codes, and the figure title
   compares paint against the second of those. */
const rankCore = y => D.composite
  .filter(c => c.year === y && c.lq && c.register === "core").sort((a, b) => b.lq - a.lq);
const runnerUp = rankCore(LATEST)[1];
/* Persistence, not a scan: paint runs above every other series drawn here in EVERY year,
   so the ranking is not the winner of a 3,000-cell lottery (SKILL stage 3). The margin is
   against the next LINE on the chart, which is what the annotation says. */
const leadYears = YEARS.filter(y => rankIn(y)[0].naics === "3255");
const leadRatio = Math.min(...YEARS.map(y => rankIn(y)[0].lq / rankIn(y)[1].lq));
/* How much longer paint's bar is than the next one in the cluster ranking, printed in the
   cold open's text alternative. It was "about four times" there for as long as the strip
   existed, and it was never four: the inherited number nobody re-derived. */
const clusterLead = paint.lq / runnerUp.lq;
/* Reader names for an industry inside a sentence. SHORT is for a chart's name gutter and
   WORD for a plot label; neither is prose ("All plastics", "plastics & rubber"). */
const PROSE = {"325": "chemical manufacturing", "3252": "resin",
  "3255": "paint and coatings", "326": "plastics and rubber products",
...
PV.figures([
  /* "strongest of the six" was the same defect as the cold open's ranking: three of the
     six are a slice of a fourth or sit outside the cluster. The card ranks the three that
...
/* ------------------------------------------------------- the cold open, in the hero
   ...
   ONLY chart a reader meets before any prose, so it carries its own reading in its own
   labels, and it is deliberately a POORER view than anything below it - no county, no
   employment, no eleven-year path - because its job is the shape and the handoff, not
   the argument.

   IT IS NOT ONE RANKING OF SIX, AND IT SHIPPED AS ONE. A reader given only the rendered
   page reported ranking six things that are not comparable, and they were right: rubber
   products and plastics products sit INSIDE plastics and rubber products, which the page
   says four screens further down would count the same jobs twice if added together, and
   chemical manufacturing is declared outside the cluster altogether. The number beside
   each name was correct; the comparison between them did not exist. Three labelled
   blocks now, and the RANKING is the first block only: the three codes the page counts
   as the cluster. The other three are drawn hollow, a different class of mark rather
   than a shorter bar in the same rank, and each block says in its own heading why it is
   not in the ranking above it. The heatmap already ruled its parent column off from its
   two slices; this is the same rule, applied at the top of the page rather than in the
   middle of it.
...
function drawOpen() {
  ...
  /* THREE BLOCKS, AND MEMBERSHIP IS READ FROM THE DATA FILE'S OWN REGISTER, never typed
     here: reclassify a code in the file and it moves block, and the heading above it is
     still true. Ranked inside each block, and the counts are COUNTED, so if the bureau
     ever withheld one at the regional level the row would drop out and the words above it
     would follow rather than printing a number the picture no longer shows. */
  const block = reg => D.naics.filter(n => n.register === reg)
    .map(n => ({code: n.code, name: SHORT[n.code], lq: comp(n.code, LATEST)?.lq, reg}))
    .filter(r => r.lq).sort((a, b) => b.lq - a.lq);
  const core = block("core");
  const BLOCKS = [
    [core, `The ${word(core.length)} industries counted as the cluster`],
    [block("detail"), "Already counted inside plastics and rubber"],
    [block("context"), "Outside the cluster, drawn for comparison"]
  ].map(([rows, head]) => ({head, rows})).filter(b => b.rows.length);
  ...
  const HI = Math.max(...ALL.map(r => r.lq)) * 1.09;

  /* THE CANVAS HEIGHT FALLS OUT OF THE BLOCKS rather than being typed. The strip used to
     carry a constant 250 for six rows in one stack; a typed height and a variable row
     count is how an axis ends up printed over a bar. */
  const ROW = M ? 36 : 22, BAR = M ? 11 : 12, HEADH = 18, GAP = M ? 12 : 10;
  const mt = M ? 74 : 60, ml = M ? 10 : 150, mr = M ? 10 : 62;
  const body = BLOCKS.reduce((s, b, i) =>
    s + HEADH + b.rows.length * ROW + (i ? GAP : 0), 0);
  const AXIS = mt + body + (M ? 9 : 7);
  const TICK = AXIS + 16;
  const FOOT = TICK + (M ? 38 : 22) + 12;
  const W = rail, H = FOOT;
  const m = {t: mt, r: mr, b: H - mt, l: ml};
  ...
  /* The reading, in the strip's own words, because this is the only chart on the page a
     reader meets before any prose. It names the CLUSTER, which the first block heading
     defines one line below it, rather than a count of everything drawn. Measured, the
     wide rail holds about 78 characters at 14px and the narrow one about 40 at 13px,
     which is why the second sentence breaks in two on the phone rather than being set
     smaller. */
  txt(svg, `Paint leads the cluster at ${fx(paint.lq)}, ${LATEST}`,
  ...
  /* One y-cursor down the blocks, so nothing below a block has to know how many rows sat
     in it. Each block records the span its rows occupy, which is what the reference rule
     is drawn against afterwards. */
  const spans = [], tops = [];
  let y = m.t;
  BLOCKS.forEach((b, bi) => {
    if (bi) y += GAP;
    txt(svg, b.head, {x: M ? m.l : 8, y: y + 13, "font-size": 13, "font-weight": 700,
      fill: bi ? O_MUTE : "#fff", opacity: bi ? 1 : .82});
    y += HEADH;
    const from = y;
    b.rows.forEach(r => {
      const top = y, on = r.code === "3255", rank = bi === 0;
      tops.push(top);
      const ink = on ? O_PAINT : rank ? O_BAR : O_MUTE;
      /* THE THREE THAT ARE NOT IN THE RANKING ARE HOLLOW. A shorter bar in the same ink
         is another entry in the same rank, which is exactly the reading this chart had to
         stop supporting: rubber products at 2.44x is LONGER than two of the three cluster
         bars, and a reader who takes the six as one list ends with the region's second
         industry being a slice of its third. Outline is a different class of mark. */
      const bx = {x: X(0), y: top + (M ? 18 : 3),
        width: Math.max(2, X(r.lq) - X(0)), height: BAR, rx: 1};
      el("rect", rank ? {...bx, fill: ink}
        : {...bx, fill: "none", stroke: ink, "stroke-width": 1.4}, svg);
      ...
      txt(svg, r.name, {x: M ? m.l : 8, y: top + (M ? 12 : 14),
        ...
        fill: on ? O_PAINT : rank ? "#fff" : O_MUTE});
      txt(svg, fx(r.lq), M ? {x: m.l + w, y: top + 12, "text-anchor": "end",
          "font-size": 14, "font-weight": rank ? 900 : 700, fill: ink}
        : {x: X(r.lq) + 9, y: top + 14, "font-size": 15,
           "font-weight": rank ? 900 : 700, fill: ink});
      y += ROW;
    });
    spans.push([from, y]);
  });
  ...
  /* THE 1.0x RULE, DRAWN LAST so it paints over the bars rather than under them: it is
     the line the whole page argues about, and every bar crossing it is the second thing
     the strip says for free.

     NEVER ONE FULL-HEIGHT LINE, and the segment is a different unit at each width because
     the obstacle is. On the phone the names sit ABOVE their own bars at x=10 while parity
     lands at x=62, so the rule is one segment per BAR: a block-length segment there runs
     straight through the two names below the first, which is the same defect as the
     single full-height rule this replaced, and it is visible only by rendering at 390.
     On the desktop form the names sit in a gutter the rule never reaches and only the
     block HEADINGS cross it, so the segment is one per BLOCK, starting under the heading.
     A data-positioned rule and a margin-positioned label meet only at some widths. */
  (M ? tops.map((t, i) => [t + 17, i === tops.length - 1 ? AXIS : t + BAR + 21])
     : spans.map(([a, b], i) => [a, i === spans.length - 1 ? AXIS : b - 2]))
    .forEach(([y1, y2]) => el("line", {x1: X(1), y1, x2: X(1), y2, stroke: O_REF,
      "stroke-width": 2}, svg));
...
function drawTrendDesktop() {
  ...
  /* NOT "all six industries". Two of the six lines are slices of a third and one is drawn
     for comparison from outside the cluster, so "six industries" asserts a comparison that
     does not exist. What IS true, and what the picture shows, is that paint runs above
     every line drawn here in every year. */
  const s1 = `Paint has run above every other line in all ${leadYears.length} years,`;
  ...
  const s2 = `always at least ${leadRatio.toFixed(1)} times the next line.`;
...
function drawTrendMobile() {
  /* THE AXIS CORNER, MEASURED. frame() sets its x-tick row 16 units below the y-tick
     baseline, which clears the 16.8-unit line box the desktop face paints by less than a
     unit and does not clear the 18-unit box the shared sheet switches to below 760px. So
     "0.0×" and "2015", which share that corner, printed on each other at every width in
     the phone band — visible to the gate only from 480 up, where the render scale finally
     pushed the same constant overlap past its 3px floor. The row is placed here instead,
     one measured leading under the bottom of the tick row frame() actually drew, and the
     canvas height falls out of where that lands rather than out of a typed 392. */
  const node = document.getElementById("trend");
  const T = face(node, "pv-tick");
  const W = 375, m = {t: 96, r: 14, b: 0, l: 38}, h = 250;
  /* Provisional: the real height is not known until the year row has been placed. */
  const {svg} = PV.chart("trend", {W, H: m.t + h + 46});
  const w = W - m.l - m.r;
  const {maxY} = trendGeom();
  const xs = y => m.l + ((y - YEARS[0]) / (YEARS.at(-1) - YEARS[0])) * w;
  const ys = v => m.t + h - (v / maxY) * h;
  frame(svg, {x: m.l, y: m.t, w, h, xs, ys,
    xt: [], yt: ticks(0, maxY, 4), yfmt: fx, xlab: "", ylab: ""});
  ...
  YEARS.filter(y => y % 5 === 0).forEach(v => txt(svg, v,
    {x: xs(v), y: yearY, "text-anchor": "middle", class: "pv-tick"}));
  m.b = Math.ceil(yearY + T.descent + 6) - (m.t + h);
  svg.setAttribute("viewBox", `0 0 ${W} ${m.t + h + m.b}`);
  /* The takeaway sits above the plot, in the first paint. */
  txt(svg, `Paint runs above every line here,`, {x: m.l - 2, y: 30, class: "pv-lab"});
...
function trendCopy() {
  ...
  /* The comparison is against the SECOND OF THE THREE the page counts as the cluster, and
     the title says which set it is ranking. It used to read "more than double the 2.44× of
     rubber products" without saying that rubber products is a slice of a line drawn on the
     same chart. */
  document.getElementById("trendtitle").textContent =
    ...
    `of ${PROSE[runnerUp.naics]}, the next of the ${word(CORE.length)}`;
...
function drawHeatMobile() {
  ...
  /* NOTHING BELOW IS A TYPED NUMBER, because none of it survived being one. The phone grid
     was authored at a 320-unit minimum with a 92-unit gutter and six columns, which leaves
     each column 36 units to hold a reading that prints 44 — so at a 360px screen "22.7×"
     ran into "6.5×" in the row beside it, the Cuyahoga callout and the last legend label
     ran past the page column, and the angled headers overlapped outright. The pitch, the
     gutter, the header band and the foot are all derived from the strings this grid
     actually prints, in the viewBox's own units, and the canvas is then never narrower than
     the widest of them. Found by collide.mjs --sweep on 2026-08-28. */
  const LAB = face(node, "pv-lab"), Q = face(node, "pv-labq");
  const AIR = 6;                     // between two readings standing side by side
  const reading = (county, n) => {
    const c = cell(county, n, LATEST);
    return c && c.lq != null ? c.lq.toFixed(1) + "×" : null;
  };
  /* A pitch picked off the average spills the wide readings: "22.7×" is nine units wider
     than "6.5×" in the same class, and the pair that has to clear is the widest ADJACENT
     pair, not the widest reading twice. */
  let pitch = 0;
  rows.forEach(a => {
    const r = NAICS.map(n => {
      const s = reading(a.name, n);
      return s ? strW(node, s, "pv-lab") : 0;
    });
    r.forEach((x, i) => {
      pitch = Math.max(pitch, x);                            // a reading fits its own cell
      if (i) pitch = Math.max(pitch, (r[i - 1] + x) / 2 + AIR);
    });
  });
  const gutter = Math.ceil(Math.max(...rows.map(a => strW(node, a.name, "pv-lab"))) + 10);
  /* VERTICAL, NOT ANGLED — the laborshed matrix's header, for the reason found here. At
     45° a label's AXIS-ALIGNED footprint grows with its own length in BOTH directions, so
     "All plastics & rubber" claimed a hundred units of width whatever the column pitch was
     and printed across its neighbours at every width in the band. Upright, the footprint
     is one line box wide however long the word is, so a header clears its own column by
     construction. The whole word is still there; truncating it to "Plastics" beside its own
     child is the defect the angle was introduced to fix, and that stays fixed. */
  const headLen = Math.max(...D.naics.map(n => strW(node, HEAD1[n.code], "pv-labq")));
  const HEAD_AIR = Math.ceil(Q.descent + 6);
  const m = {t: Math.ceil(headLen + HEAD_AIR + 4), r: 6, b: 0, l: gutter};

  /* The foot is four stacked things — the callout, its reading, what the ramp direction
     means, and two swatch rows — so each one hangs a measured leading off the one above it
     and the bottom margin is wherever the last of them ends. */
  const dCall = Math.ceil(LAB.ascent + 14);
  const dSub = dCall + lead(node, "pv-lab", "pv-labq");
  const dRamp = dSub + lead(node, "pv-labq", "pv-labq") + 6;
  const SW = 18;                                          // the legend swatch
  const dKey1 = Math.ceil(dRamp + Q.descent + 8);
  const dKey2 = dKey1 + SW + 8;
  m.b = Math.ceil(dKey2 + SW + 8);

  /* WIDENING THE MINIMUM IS A ZOOM AND ENLARGING TYPE IS NOT: marks and labels scale
     together, so nothing that reads at 1:1 can newly meet. The minimum is whichever of the
     grid and the foot lines runs widest, measured, and the phone still renders it into the
     column it has. */
  const legStep = Math.ceil(
    22 + Math.max(...LEGEND.map(([, s]) => strW(node, s, "pv-tick"))) + 12);
  const callout = `Outlined: Cuyahoga paint, ${fx(cuyPaint.lq)} on ${N(cuyPaint.emp)} jobs,`;
  const sub = `${Math.round(cuyShare * 100)}% of the region’s disclosed paint jobs`;
  const ramp = "darker cells are more concentrated";
  const keyRun = k => 6 + (k.length - 1) * legStep + 22 + strW(node, k.at(-1)[1], "pv-tick") + 6;
  const minW = Math.ceil(Math.max(
    m.l + NAICS.length * pitch + m.r,
    6 + strW(node, callout, "pv-lab") + 6,
    6 + strW(node, sub, "pv-labq") + 6,
    6 + strW(node, ramp, "pv-labq") + 6,
    keyRun(LEGEND.slice(0, 4)), keyRun(LEGEND.slice(4))));
  const W = Math.max(minW, Math.round(
    node.parentElement.getBoundingClientRect().width) || minW);

  const {svg, h, w} = PV.chart("heat",
    {W, rows: rows.length, rowH: Math.ceil(LAB.h + 12), m});
  suppPattern(svg);
  const cw = w / NAICS.length, ch = h / rows.length;
  const cx = i => m.l + i * cw + cw / 2;
  /* A line rotated -90° about its anchor runs upward from it, with the line box spanning
     one ascent left of the anchor and one descent right — so the anchor is offset by half
     their difference to sit the box on the column's centre. */
  D.naics.forEach((n, i) => {
    const x = cx(i) + (Q.ascent - Q.descent) / 2, y = m.t - HEAD_AIR;
    txt(svg, HEAD1[n.code], {x, y, "text-anchor": "start", class: "pv-labq",
  ...
  [m.l + pi * cw, m.l + (pi + 1) * cw].forEach(x =>
    el("line", {x1: x, y1: m.t - 4, x2: x, y2: m.t + h, stroke: "var(--pv-axis)",
      "stroke-width": 1.5}, svg));
  /* A reading sits on the optical centre of its cell, which is half the ascent-to-descent
     run below the middle, not a typed five units under it. */
  const midline = top => top + (ch + LAB.ascent - LAB.descent) / 2;

  rows.forEach((a, r) => {
    txt(svg, a.name, {x: m.l - 8, y: midline(m.t + r * ch), "text-anchor": "end",
      class: "pv-lab"});
    NAICS.forEach((n, c) => {
      const cl = cell(a.name, n, LATEST);
      const x = m.l + c * cw + 1, y = m.t + r * ch + 1;
      ...
      /* The × goes IN the cell. Every other number on the page carries it, and a reader
         who lands on the grid first has no way to tell whether a bare 22.7 is a multiple,
         a percentage or a job count in hundreds. */
      if (ok) txt(svg, cl.lq.toFixed(1) + "×", {x: x + cw / 2 - 1, y: midline(y - 1),
        "text-anchor": "middle", class: "pv-lab",
        fill: cl.lq >= 11 ? "#fff" : "var(--pv-ink)"});
      hoverable(el("rect", {x, y, width: cw - 2, height: ch - 2, fill: "transparent"}, svg),
        ok ? `<b>${a.name} County · ${cl.label}</b><br><span class="v">${fx(cl.lq)}</span>
  ...
  const floor = m.t + h;
  txt(svg, callout, {x: 6, y: floor + dCall, class: "pv-lab", fill: "var(--hover)"});
  txt(svg, sub, {x: 6, y: floor + dSub, class: "pv-labq"});
  txt(svg, ramp, {x: 6, y: floor + dRamp, class: "pv-labq"});
  heatLegend(svg, LEGEND.slice(0, 4), 6, floor + dKey1, legStep);
  heatLegend(svg, LEGEND.slice(4), 6, floor + dKey2, legStep);
  heatCopy(rows);
}
...
function drawScatterDesktop() {
  ...
  /* THE RIGHT-HAND LABEL, MOVED OFF ITS NEIGHBOUR AND STACKED OVER ITS OWN DOT. Set on
     one line 13 units above the point it names, this 39-character string ran left across
     a third of the plot and its plate closed on Summit's plastics and rubber dot at
     3.27x, which is exactly what a reader given only the rendering reported: "Cuyahoga
     chemicals, 1.46x" printed beside a dot sitting at about three and a half. Two shorter
     lines stacked above the point fix it, and the fix is a MEASURED one rather than a
     nudge: the rightmost mark other than this one is 46 units left of where the shorter
     lines begin, so the label stands in an empty column over its own dot at every width.
     A first attempt kept the long first line and only lifted it, which cleared the 3px
     collision floor and still printed the words directly above the wrong dot. */
  const bbx = xs(bigBase.emp), bby = ys(bigBase.lq);
  plated(svg, `${bigBase.name} ${WORD[bigBase.naics]}`, bbx - 11, bby - 30,
    "pv-lab", "var(--pv-ink)", 7.3, "end");
  plated(svg, `${fx(bigBase.lq)} on ${N(bigBase.emp)} jobs`, bbx - 11, bby - 14,
    "pv-labq", "var(--pv-muted)", 7.0, "end");
}
...
document.getElementById("footcorr").innerHTML =
  `Correction: an earlier version of this page called Cuyahoga&rsquo;s paint cell the
   ...
   <br>Correction, 29 August 2026: this page called paint the strongest of six polymer
   industries, and drew the six as one ranked strip. Two of the six, plastics products and
   rubber products, sit inside the third, and chemical manufacturing sits outside the
   cluster, so the six were never one ranking. Paint is the strongest of the
   ${word(CORE.length)} the page counts as the cluster; the opening strip now draws those
   ${word(CORE.length)} as the ranking and sets the other ${word(NAICS.length - CORE.length)}
   apart under headings that say why they are not in it. The same version said paint&rsquo;s
   bar ran about four times further right than any other. It runs
   ${clusterLead.toFixed(2)} times the next bar of the ${word(CORE.length)} and
