# PIC portfolio timeline — historical context and the operating record

The page puts documented regional history beside PIC’s public operating record. The
history supplies context; it does not establish why a federal designation was awarded
...
| File | Role |
|-|-|
| `index.html` | Initial text, page structure, photographs and captions |
| `app.js` | Scales, packing, rendering and interaction |
| `styles.css` | Page-specific figures, panel and tables |
| `data/timeline.json` | Public projection of the internal operating-event inventory |
| `data/heritage.json` | Public historical projection, with source-linked editorial corrections |
| `claims.json` | Re-runnable checks and explicit manual-source claims |
| `img/` | Five resized archive photographs; source credits and rights statements appear on the page |

## Historical projection and corrections

The original producer, `tools/build-heritage-web.py`, belongs to the internal PIC-decks
project and is not present in this public checkout. It reads the NEO Polymer Wins
register, selects heritage and discovery rows labeled PROVEN, and excludes other
collections and internal references. The label describes the register’s editorial
status; it does not certify universal scientific priority.

The public projection merges the duplicate Case department event and separates Kent’s
institute founding from its later research paper. The September 2026 source review also
moves Ameripol’s 1940 product introduction into heritage, corrects paper attribution,
and narrows unsupported historical wording. Original IDs remain intact. These are
public projection corrections; the internal register has not been edited here. Before
replacing this snapshot from an internal rebuild, reconcile the corrections in
[`CORRECTIONS.md`](../CORRECTIONS.md) and run the claim checks. Blind regeneration would
lose the editorial corrections.

The historical axis uses equal-width eras with linear dates inside each era:
1898–1919, 1920–1945, 1946–1969, 1970–1999 and 2000–2012. A sixth block covers 2013
through the 2023 designation. Existing event spans reach 2016; the absence afterward
belongs to this selected, documented projection, not to the whole register or regional
history. No distinct event is counted twice to populate both chart rows.

## Photographs

Each caption links its archive record. The Library of Congress records state “No known
restrictions on publication”; the other images carry Navy and NASA credits. The page
does not turn those statements into a blanket public-domain certification.

| File | Subject and source |
|-|-|
| `USN-AKRON-CONSTRUCTION-1930.jpg` | USS Akron construction, 5 November 1930; NHHC NH 42164 |
| `LOC-PALMER-AIRDOCK-COLOR-1941.jpg` | Goodyear Airdock housing aircraft subassembly shops, December 1941; Alfred T. Palmer, LOC fsac.1a35066 |
| `LOC-PALMER-GOODYEAR-TANK-1941.jpg` | Aircraft fuel-tank manufacture, December 1941; Alfred T. Palmer, LOC fsac.1a35064 |
| `LOC-PALMER-GOODRICH-1941.jpg` | Ameripol sheet at a rolling mill, 1941; Alfred T. Palmer, LOC fsac.1a35068 |
| `NASA-AEROGEL-POLYMER-2011.jpg` | Polymer-reinforced aerogel supporting jars of coins; NASA |

No cleared university-milestone photograph has been identified by this register. That
is a bounded search result, not a statement that no archive holds one.
...
# From the repository root, open http://localhost:8900/timeline/
```

## Regenerating the operating record

This is an internal PIC-decks command, not a script in the public checkout:
...
The dated revision notes below preserve earlier designs and superseded figures. The
current projection and corrections above take precedence.
